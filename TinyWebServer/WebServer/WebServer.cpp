...
void WebServer::createServerSocket()
{
    ...
    this->epollFd = epoll_create(5);//创建存放事件连接的文件描述符
    //断言
    assert(this->epollFd != -1);

    //同时设置Task类和ListTimer里的epollFd
    ListTimer::ltEpollFd = this->epollFd;
    Http::hEpollFd = this->epollFd;

    //注册事件
    this->addFdToEpoll(this->serverSocket, true, false);
...
bool WebServer::createSignal()
{
    ...
    //添加epoll读事件
    this->addFdToEpoll(pipeFd[0], false, false);
...
void WebServer::epollWait()
{
    ...
    {
        int count = epoll_wait(this->epollFd, this->epEvents, MAXEVENT, -1);
        ...
        for(int i = 0; i < count; i++)
        {
            ...
            else if(this->epEvents[i].events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR))
            {
                //服务器端关闭连接
            ...
            else if((sockFd == this->pipeFd[0]) && (this->epEvents[i].events & EPOLLIN))
            {
                //处理信号
